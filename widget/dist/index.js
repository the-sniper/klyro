"use strict";var M=(Y,E,y)=>new Promise((x,R)=>{var m=a=>{try{h(y.next(a))}catch(w){R(w)}},e=a=>{try{h(y.throw(a))}catch(w){R(w)}},h=a=>a.done?x(a.value):Promise.resolve(a.value).then(m,e);h((y=y.apply(Y,E)).next())});(function(){"use strict";let Y="2.3.3",E=null,y="",x="",R={init:m};function m(){return M(this,arguments,function*(o={}){if(typeof o=="string"&&(o={key:o}),E=o.key||o.widgetKey,!E){console.error("[Klyro] Missing widget key in initKlyro");return}y=o.apiBase||"https://klyro-pro.vercel.app",x=`klyro_${E}`,console.log("[Klyro] Initializing with version:",Y),document.readyState==="loading"?document.addEventListener("DOMContentLoaded",X):X()})}let e=null,h=!1,a=[],w=null,v=!1,f=null,j=null,G=!1,k=!1;function W(){try{let o=localStorage.getItem(x);if(o){let r=JSON.parse(o);return a=r.messages||[],w=r.sessionId||null,console.log("[Klyro] Loaded persisted chat:",a.length,"messages"),!0}}catch(o){console.error("[Klyro] Failed to load persisted chat:",o)}return!1}function F(){try{let o={messages:a,sessionId:w,lastUpdated:new Date().toISOString()};localStorage.setItem(x,JSON.stringify(o))}catch(o){console.error("[Klyro] Failed to persist chat:",o)}}function Z(){try{localStorage.removeItem(x)}catch(o){console.error("[Klyro] Failed to clear persisted chat:",o)}}function q(){if(a.length===0)return;let o=(e==null?void 0:e.headerTitle)||"Chat Transcript",r=new Date().toLocaleString(),t=`${o}
`;t+=`Downloaded: ${r}
`,t+="=".repeat(50)+`
    ...
    `,document.body.appendChild(t),f=t;let g=t.querySelector(".klyro-button"),p=t.querySelector(".klyro-panel"),d=t.querySelector(".klyro-messages"),l=t.querySelector(".klyro-input"),c=t.querySelector(".klyro-send"),u=t.querySelector(".reset-btn"),N=t.querySelector(".klyro-header"),ao=t.querySelector(".menu-btn"),D=t.querySelector(".klyro-menu-popover"),H=t.querySelector(".expand-toggle-btn"),K=t.querySelector(".download-transcript-btn"),Q=t.querySelector(".klyro-popover-overlay"),no=Q.querySelector(".confirm"),lo=Q.querySelector(".cancel");I(),g.addEventListener("click",()=>{h=!h,p.classList.toggle("open",h);let s=e.launcherMode==="text"&&e.launcherText;h?(g.innerHTML=V,g.classList.remove("text-mode"),l.focus(),I()):(g.innerHTML=S+(s?`<span class="klyro-button-text">${B(e.launcherText)}</span>`:""),s&&g.classList.add("text-mode"))}),u.addEventListener("click",()=>{k=!1,D.classList.remove("open"),Q.classList.add("open")}),ao.addEventListener("click",s=>{s.stopPropagation(),k=!k,D.classList.toggle("open",k)}),document.addEventListener("click",()=>{k&&(k=!1,D.classList.remove("open"))}),H.addEventListener("click",()=>{G=!G,p.classList.toggle("expanded",G);let s=H.querySelector(".klyro-menu-item-icon"),b=H.querySelector(".klyro-menu-item-text");G?(s.innerHTML=oo,b.textContent="Collapse window"):(s.innerHTML=U,b.textContent="Expand window"),k=!1,D.classList.remove("open")}),K.addEventListener("click",()=>{q(),k=!1,D.classList.remove("open")}),lo.addEventListener("click",()=>{Q.classList.remove("open")}),no.addEventListener("click",()=>{a=[],w=null,Z(),I(),Q.classList.remove("open")});function T(s){return M(this,null,function*(){let b=s||l.value.trim();if(!(!b||v)){a.push({role:"user",content:b}),s||(l.value=""),I(),F(),v=!0,I();try{let A=yield fetch(`${y}/api/chat`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:b,widgetKey:E,sessionId:w})}),C=yield A.json();v=!1,A.ok?(w=C.sessionId,a.push({role:"assistant",content:C.response}),F()):a.push({role:"assistant",content:"Sorry, I encountered an error. Please try again."})}catch(A){v=!1,a.push({role:"assistant",content:"Unable to connect. Please try again later."})}I()}})}c.addEventListener("click",()=>T()),l.addEventListener("keypress",s=>{s.key==="Enter"&&T()});function I(){let s=a.filter((A,C)=>!(C===0&&A.role==="assistant"&&e.welcomeMessage&&A.content.trim()===e.welcomeMessage.trim()));s.length>0?N.classList.add("chat-active"):N.classList.remove("chat-active"),K.disabled=s.length===0,s.length===0?d.innerHTML=`
        ...
        `:d.innerHTML=s.map(A=>`
          <div class="klyro-message ${A.role}" ${A.role==="user"?`style="background: ${e.primaryColor}"`:""}>
            ${A.role==="assistant"?io(A.content):B(A.content)}
          </div>
        `).join(""),v&&(d.innerHTML+=`
        ...
        `);let b=s;if(!v&&b.length>0)if(b[b.length-1].role==="assistant"){let C=d.lastElementChild;C&&(d.scrollTop=C.offsetTop-20)}else d.scrollTop=d.scrollHeight;else d.scrollTop=d.scrollHeight;c.disabled=v}}function io(o){if(!o)return"";console.log("[Klyro] formatMessage called with:",o.substring(0,100));let r=[],t=o.replace(/\[([^\]]+)\]\((https?:\/\/[^\)]+)\)/g,(l,c,u)=>{let N=`__LINK_${r.length}__`;return r.push({text:c,url:u}),N}),n=[];t=t.replace(/(https?:\/\/[^\s<\]]+)/g,(l,c)=>{if(l.includes("__LINK_"))return l;console.log("[Klyro] Found raw URL:",c);let u=`__URL_${n.length}__`;return n.push(c),u});let i=B(t);r.forEach((l,c)=>{let u=B(l.text);i=i.replace(`__LINK_${c}__`,`<a href="${l.url}" target="_blank" rel="noopener noreferrer">${u}</a>`)}),n.forEach((l,c)=>{i=i.replace(`__URL_${c}__`,`<a href="${l}" target="_blank" rel="noopener noreferrer">${l}</a>`)}),i=i.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),i=i.replace(new RegExp("(?<!^|\\n)\\*([^*\\n]+)\\*","g"),"<em>$1</em>");let g=i.split(`
`),p=!1,d=[];for(let l=0;l<g.length;l++){let c=g[l],u=c.match(/^[\s]*[•\-\*]\s+(.+)$/);u?(p||(d.push("<ul>"),p=!0),d.push(`<li>${u[1]}</li>`)):(p&&(d.push("</ul>"),p=!1),d.push(c))}return p&&d.push("</ul>"),i=d.join(`
`),i=i.replace(/\n(?!<\/?[uo]l|<\/?li)/g,"<br>"),i=i.replace(/\n/g,""),console.log("[Klyro] formatMessage result:",i.substring(0,100)),i}function B(o){if(typeof document=="undefined")return o;let r=document.createElement("div");return r.textContent=o,r.innerHTML}if(typeof document!="undefined"){let o=document.currentScript;if(o){let r=o.getAttribute("data-widget-key"),t=o.getAttribute("data-api-base");if(r){let n=t;if(!n&&o.src)try{let i=new URL(o.src);!i.hostname.includes("unpkg.com")&&!i.hostname.includes("jsdelivr.net")&&(n=i.origin)}catch(i){n=o.src.replace("/widget.js",""),(n==="widget.js"||n==="")&&(n=window.location.origin)}m({key:r,apiBase:n})}}}typeof module!="undefined"&&module.exports?module.exports={initKlyro:m}:window.initKlyro=m})();
